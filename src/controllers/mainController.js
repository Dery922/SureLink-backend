...
// Public marketplace catalog: paginated, filterable list of completed providers.
// Ported from Frank's authController; depends only on the User model.
export async function getMarketplaceProviders(req, res, next) {
  try {
    ...
    const [providers, totalCount] = await Promise.all([
      User.find(query)
        .select("-security -verification -id_number -id_doc_url")
        .sort({ "trust.score": -1, createdAt: -1 })
        .skip(skipAmount)
        .limit(parseInt(limit))
        .lean(),
