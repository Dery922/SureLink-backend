...
async function createOtpFlow(adminId) {
  ...
  const existingHash = await adminSessionRepository.findExistingPending(String(adminId));
  if (existingHash) {
    await adminSessionRepository.deleteOtpPending(existingHash, String(adminId));
  }
...
export async function verifyAdminOtp({ pending_token, otp, ip, user_agent }) {
  ...
  if (pending.attempts >= OTP_MAX_ATTEMPTS) {
    await adminSessionRepository.deleteOtpPending(pendingTokenHash, pending.admin_id);
    throw new AppError(
