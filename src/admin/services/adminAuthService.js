...
// Lock an account after this many consecutive failed attempts.
 ...
 * AdminAuthService.
 *
 * Business logic for admin authentication. Controllers should call these
 * functions and never touch repositories or factories directly.
 *
 * AppSec notes:
 * - bcrypt compare is used for password verification (constant-time, no
 *   timing attack risk from naive string comparison).
 * - Rate-limiting via failed_login_attempts + locked_until prevents brute force.
 * - Generic error messages on login failure prevent user enumeration.
 * - The session token is only ever returned once; only its SHA-256 hash is
 *   stored in Redis.
 */
...
/**
 * Authenticate an admin with email + password and issue a session token.
 *
 * Returns a public admin shape and a session token.
 */
export async function loginAdmin({ email, password, ip, user_agent }) {
  ...
  // Fetch with password_hash included.
  const admin = await adminRepository.findByEmailWithPassword(email);

  // Use a generic message for both "not found" and "wrong password" to prevent
  // user enumeration (AppSec).
  ...
  if (admin.locked_until && admin.locked_until > new Date()) {
    const remaining = Math.ceil(
      (admin.locked_until - Date.now()) / 1000 / 60,
    );
    throw new AppError(
  ...
  // Password correct — reset attempt counter and record login.
...
/**
 * Resolve the current admin from a session token.
 *
 * Used by the /me endpoint and the authenticateAdmin middleware.
 */
